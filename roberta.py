...
from massive_dataset import MASSIVEDatasetTranslated, MASSIVEDatasetSplitName
from translator import Translator

ROBERTA_HUGGINGFACE_MODEL_ID = "roberta-base"
ROBERTA_HUGGINGFACE_TOKENIZER_ID = "roberta-base"
...
class RobertaTokenizer:
    ...
    def tokenize_batch(self, batch: Tensor) -> Tensor:
        t = self.tokenizer(batch, return_tensors="pt", padding=True)
        for key in t.keys():
            t[key] = t[key].cuda()
        return t
...
class RobertaModel:
    def __init__(self, model: AutoModel, tokenizer: AutoTokenizer, first_translate=False):
        ...
        self.tokenizer = tokenizer
        self.first_translate = first_translate
        if self.first_translate:
            self.translator = Translator()

    def __call__(self, x: Tensor) -> Tensor:
        if self.first_translate:
            x = self.translator.translate(x) #translate from polish to english if needed
...
class RobertaMASSIVEIntentClassifier(nn.Module):
    def __init__(
        self,
        hidden_size: int,
        num_of_layers: int = 3,
...
class RobertaExperiment(Experiment):
    def __init__(
        self,
        ...
        super(RobertaExperiment, self).__init__(
        ...
        self.dataset = MASSIVEDatasetTranslated()
        ...
        self.model = RobertaModel(bare_model, RobertaTokenizer(bare_tokenizer))
        intent_clf = RobertaMASSIVEIntentClassifier(
